...
class GenMessageQueue(CfgCLI):
    ...
    LOGO = '/conf/gen_message_queue.logo'
    ...
    def __init__(self, verbose=False):
        ...
        gen_message_queue_property = {
            'ats_organization': 'vroncevic',
            'ats_repository': 'gen_message_queue',
            'ats_name': 'gen_message_queue',
            'ats_logo_path': '{0}{1}'.format(
                current_dir, GenMessageQueue.LOGO
            ),
            'ats_use_github_infrastructure': True
        }
        splash = Splash(gen_message_queue_property, verbose=verbose)
        base_info = '{0}{1}'.format(current_dir, GenMessageQueue.CONFIG)
        CfgCLI.__init__(self, base_info, verbose=verbose)
        verbose_message(GenMessageQueue.GEN_VERBOSE, verbose, 'init tool info')
        self.logger = ATSLogger(
            GenMessageQueue.GEN_VERBOSE.lower(),
            '{0}{1}'.format(current_dir, GenMessageQueue.LOG),
            verbose=verbose
        )
        if self.tool_operational:
            ...
            self.add_new_option(
                GenMessageQueue.OPS[4], action='version', version=__version__
            )
    ...
    def process(self, verbose=False):
        ...
        if self.tool_operational:
            ...
            if num_of_args_sys > 1:
                ...
                if operation not in GenMessageQueue.OPS:
                    sys.argv.append('-h')
            else:
                sys.argv.append('-h')
            ...
            if not pro_exists:
                if bool(getattr(args, 'gen')):
                    ...
                    generator = MessageQueue(verbose=verbose)
                    status = generator.gen_setup(getattr(args, 'gen'), verbose)
                    if status:
                        ...
                        error_message(
                            GenMessageQueue.GEN_VERBOSE, 'generation failed'
                        )
                        self.logger.write_log(
                    ...
                    error_message(
                        GenMessageQueue.GEN_VERBOSE, 'provide project name'
                    )
                    self.logger.write_log(
                ...
                error_message(
                    GenMessageQueue.GEN_VERBOSE, 'project already exist'
                )
                self.logger.write_log(
            ...
            error_message(
                GenMessageQueue.GEN_VERBOSE, 'tool is not operational'
            )
            self.logger.write_log(
    ...
    def __str__(self):
        '''
            Dunder method for GenMessageQueue.
