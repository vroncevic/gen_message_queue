...
Copyright
    Copyright (C) 2022 - 2026 Vladimir Roncevic <elektron.ronca@gmail.com>
...
class ReadTemplateTestCase(TestCase):
    ...
    def test_read_template_empty(self) -> None:
        ...
        with self.assertRaises(ATSValueError):
            self.assertFalse(
                bool(template_read.read(templates, 'simple_read', 'posix'))
            )
    ...
    def test_read_template(self) -> None:
        ...
        self.assertTrue(bool(
            template_read.read(
                yml2obj.read_configuration(), 'simple_read', 'posix'
            )
        ))
